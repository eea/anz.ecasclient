...
class ECas20ServiceTicketValidator(Cas20ServiceTicketValidator):
    ...
    def __init__(
        ...
        if renew is None:
            super(ECas20ServiceTicketValidator, self).__init__(
                casServerUrlPrefix, pgtStorage
            )
        else:
            super(ECas20ServiceTicketValidator, self).__init__(
                casServerUrlPrefix, pgtStorage, renew
            )
    ...
    def _constructValidationUrl(self, ticket, service, proxyCallbackUrl):
        ...
        url = super(
            ECas20ServiceTicketValidator, self
        )._constructValidationUrl(ticket, service, proxyCallbackUrl)
        self.casServerUrlPrefix = saveUrl
        return url

    def parseResponseFromServer(self, response):
        try:
            casPrincipalAs = super(
                ECas20ServiceTicketValidator, self
        ...
        meta = {"user": None, "moniker": None, "authenticationLevel": None}
        for e in meta.keys():
            elements = dom.getElementsByTagNameNS(self.CAS_NS, e)
            meta[e] = elements and elements[0].firstChild.data or None
        setattr(
            casPrincipalAs.principal, "ecas_id", casPrincipalAs.principal.id
        )
        setattr(casPrincipalAs.principal, "meta", meta)
